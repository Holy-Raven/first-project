...
приступаем к поиску наибольшего общего префикса, для этого используем метод эффективного поиска шаблона в тексте search.
Сравниваем первую распакованную строку с остальными, постепенно сокращая предполагаемый префикс, пока он не станет подходящим
для всех строк.
...
случае может занять O(n * m^2). Общую сложность можно принять как O(n * m^2). Где n - количество строк, m - средняя длина
распакованных строк.
